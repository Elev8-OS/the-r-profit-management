...
- [x] 51 real listings imported into `/listings` — a one-time join of live PriceLabs (52 rows) and Elev8 Suite (72 rows) data on their shared Channex UUID (Elev8's `listing_id` == the prefix of PriceLabs' `listing_id` before `___`, a **confirmed** match, not a heuristic). 43 matched in both systems; 8 exist only in PriceLabs (4× MiHome-branded units, Villa Toba, 3× "The R Loft" rooms) with no Elev8 counterpart yet. Runs automatically via `preDeployCommand` (`packages/db/prisma/seedListings.ts`), idempotent. Dashboard/KPI numbers are still empty — this only imports the listing *objects*, not revenue/pricing history.
- [ ] Real PriceLabs/MyDataValue/Elev8 API clients (Phase 1) — MDV confirmed scoped to account_name "Reto Wyss"; Elev8 has a service key (`ELEV8_API_KEY`, set on `worker-app`) but its base URL/auth scheme are still unconfirmed (see `packages/integrations/src/elev8/client.ts`). PriceLabs/MDV real REST API keys (`PRICELABS_API_KEY`, `MDV_API_KEY`) are not set yet either — needed before any revenue/pricing data can sync in.
- [ ] Ongoing cross-system listing ID reconciliation job (Phase 1) — the one-time import above isn't a live sync; new/changed listings in PriceLabs/Elev8 won't appear automatically yet
